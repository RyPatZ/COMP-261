...
public class Assignment5 {
	...
	private void initialise() {
		...
		searchField2 = new JTextField(SEARCH_COLS);
		searchField2.setMaximumSize(new Dimension(0, 25));
		searchField2.addActionListener(new ActionListener() {
										   public void actionPerformed(ActionEvent e) {
											   String pattern = searchField2.getText();
											   String text = textEditor.getText();
											   ...
											   if (index == -1) {
												   ...
												   textEditor.setSelectionStart(index);
												   textEditor.setSelectionEnd(index + pattern.length());
												   textEditor.setSelectionColor(Color.RED);
											   }
										   }
									   }
		);

		searchField5 = new JTextField(SEARCH_COLS);
		searchField5.setMaximumSize(new Dimension(0, 25));
		searchField5.addActionListener(new ActionListener() {
										   public void actionPerformed(ActionEvent e) {
											   String pattern = searchField5.getText();
											   String text = textEditor.getText();
		...
		controls.add(searchField2);
		controls.add(new JLabel("BoyerMooreSearch"));
		controls.add(searchField5);
		...
		// first, setup a listener to call onEditorLoad when the user loads a
		// file.
		JButton load2 = new JButton("Load ngram");
		load2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				// set up the file chooser
				fileChooser2.setCurrentDirectory(new File("."));
				fileChooser2.setDialogTitle("Select input file.");
				fileChooser2.setFileSelectionMode(JFileChooser.FILES_ONLY);

				// run the file chooser and check the user didn't hit cancel
				if (fileChooser2.showOpenDialog(frame2) == JFileChooser.APPROVE_OPTION) {
					File file = fileChooser2.getSelectedFile();
					String s = readFile(file);
					ng.Ngrams(s);
					String str = "input a n number into the first search box to see the ngram table of sepicific Ngram(n)";
					str+= "\n input number has to be <= 5 \n";
					str += "\n \n input a string into the second search box to get the log probability of the string";
					textEditor2.setText(str);
				}
			}
		});

		searchField3 = new JTextField(SEARCH_COLS);
		searchField3.setMaximumSize(new Dimension(0, 25));
		searchField3.addActionListener(new ActionListener() {
										  public void actionPerformed(ActionEvent e) {
											  String text = searchField3.getText();
											  textEditor2.setText(ng.getNgramProb(text));
										  }
									  }
		);

		searchField4 = new JTextField(SEARCH_COLS);
		searchField4.setMaximumSize(new Dimension(0, 25));
		searchField4.addActionListener(new ActionListener() {
										   public void actionPerformed(ActionEvent e) {
											   String text = searchField4.getText();
											   String p = "the text :\n ";
											   p += "               ";
											   p+= text + "\n";
											   p+= "has Log probability :" + ng.findCharProbs(text).toString()+"\n \n";
											   for (String s : ng.CharProb.keySet()){
											   	p+= "prefix = " + s + ", n= " + s.length() +":\n" ;
											   	for (Character s1 : ng.ngram.get(s.length()).get(s).keySet()){
											   		p+= "   probability of character " + s1 + ": " + ng.ngram.get(s.length()).get(s).get(s1)+"\n";
												}
											   }
											   textEditor2.setText(p);
										   }
									   }
		);

		JPanel ngram = new JPanel();
		ngram.setLayout(new BoxLayout(ngram, BoxLayout.LINE_AXIS));

		Border edge2 = BorderFactory.createEmptyBorder(LAYOUT_GAP, LAYOUT_GAP, LAYOUT_GAP, LAYOUT_GAP);
		ngram.setBorder(edge2);

		ngram.add(load2);
		ngram.add(Box.createHorizontalGlue());
		ngram.add(Box.createRigidArea(new Dimension(LAYOUT_GAP, 0)));
		ngram.add(new JLabel("show ngram table of prefix number"));
		ngram.add(searchField3);
		ngram.add(new JLabel("input a string to get Log probs"));
		ngram.add(searchField4);

		textEditor2 = new JTextArea(EDITOR_ROWS, EDITOR_COLS);
		textEditor2.setLineWrap(true);
		textEditor2.setWrapStyleWord(true);
		textEditor2.setEditable(true);

		File defaultFile2 = new File(DEFAULT_EDITOR_FILE);
		if (defaultFile2.exists())
			textEditor2.setText(readFile(defaultFile));
		else
			textEditor2.setText(DEFAULT_EDITOR_TEXT);

		JScrollPane scroll2 = new JScrollPane(textEditor2);

		JPanel Ngram = new JPanel();
		Ngram.setLayout(new BorderLayout());
		Ngram.add(ngram, BorderLayout.NORTH);
		Ngram.add(scroll2, BorderLayout.CENTER);
